...
public class Steering
{
    ...
    public static Vector3 Separation(Agent agent, GameObject[] neighbors, float radius)
    {
        ...
        foreach (GameObject neighbor in neighbors)
        {
            Vector3 direction = (neighbor.transform.position - agent.transform.position - neighbor.transform.position);
    ...
    public static Vector3 Alignment(Agent agent, GameObject[] neightbors)
    {
        Vector3 averageVelocity = Vector3.zero;
        foreach (GameObject neighbor in neightbors)
        {
            averageVelocity += neighbor.GetComponent<Agent>().movement.velocity;
        }

        averageVelocity /= neightbors.Length;
